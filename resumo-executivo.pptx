...
O notebook `notebooks/03_gold/14_gold_views.ipynb` cria uma view (`vw_` + nome da tabela) para cada uma das 9 tabelas físicas listadas acima. As views ficam disponíveis no schema `case_dados` e podem ser consultadas direto pelo SQL Editor do Databricks ou por qualquer ferramenta de BI conectada ao workspace, sem necessidade de referenciar o nome físico da tabela.

O mesmo notebook imprime a contagem final de registros de cada tabela como evidência de que a pipeline rodou até o fim. A saída fica registrada no resultado da célula `display(df_resumo)` dentro do próprio notebook, no Databricks, e serve como prova de execução completa sem precisar de captura de tela.
