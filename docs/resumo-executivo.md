...
- Granularidade de item de pedido na fato principal (`fact_pedido_item`), em vez de pedido, permitindo segmentação por produto/categoria sem perder a capacidade de agregação no nível de pedido.
- Rateio de receita por item, dado que `net_amount` está disponível apenas no cabeçalho do pedido, evitando duplicação de receita ao agregar por item.
...
- Critério de deduplicação explícito em todos os casos: data de atualização mais recente, score de qualidade dos campos ou desempate manual quando os critérios anteriores não são suficientes.
...
- O campo `cost` (custo de frete) contém o valor `"unknown"` em 10 registros. Esse problema só foi identificado quando a agregação de soma falhou na camada Gold.
...
- Padronizar a captura de dados nas fontes de origem, com formato único de data e domínio fechado para campos de status, reduzindo o volume de tratamento em cargas futuras.
